...
const filePath = computed(() => {
  // relativePath is relative to srcDir (src/), so we need to prepend 'src/'
  const relativePath =
    page.value.filePath?.replace(/^\//, '') || page.value.relativePath?.replace(/^\//, '') || ''
  if (!relativePath) return ''
  // If path doesn't already start with 'src/', add it
...
const uiText = computed(() => {
  return {
    contributors: isRussianLocale.value ? 'Авторы статьи' : 'Article contributors',
    loading: isRussianLocale.value ? 'Загрузка авторов...' : 'Loading contributors...',
    error: isRussianLocale.value ? 'Ошибка' : 'Error',
    noContributors: isRussianLocale.value
      ? 'Авторы статьи не найдены'
      : 'No contributors found for this article',
    noFilePath: isRussianLocale.value ? 'Путь к статье недоступен' : 'No article path available',
  }
})

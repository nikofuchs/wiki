<script setup lang="ts">
    ...
    const filePath = computed(() => {
        const path = page.value.filePath?.replace(/^\//, '') || page.value.relativePath?.replace(/^\//, '') || ''
        return path
    })
    ...
    const uiText = computed(() => {
        return {
            ...
            noContributors: isRussianLocale.value ? 'Авторы статьи не найдены' : 'No contributors found for this article',
